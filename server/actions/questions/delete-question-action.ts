...
export async function deleteQuestionAction(id: string | number) {
  try {
    ...
    const resp = await fetch(`${url}/api/questions/${id}`, {
      method: 'DELETE',
      headers: {
    ...
    if (!resp.ok) {
      return {
        ok: false as const,
        msg: errorMessage(body, 'Error al eliminar la pregunta'),
      };
    }
    ...
    return { ok: false as const, msg: 'Error al eliminar la pregunta' };
  }
}
