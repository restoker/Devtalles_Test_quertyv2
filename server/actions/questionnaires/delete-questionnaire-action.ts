...
export async function deleteQuestionnaireAction(id: string | number) {
  try {
    ...
    if (!resp.ok) {
      return {
        ok: false as const,
        msg: errorMessage(body, 'Error al desactivar el cuestionario'),
      };
    }
    ...
    return {
      ...
      msg: 'Cuestionario desactivado exitosamente',
    };
  } catch {
    return { ok: false as const, msg: 'Error al desactivar el cuestionario' };
  }
}
