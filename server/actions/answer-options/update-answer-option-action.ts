...
export async function updateAnswerOptionAction(
  id: string | number,
  input: { label?: string; value?: string | null; sortOrder?: number }
) {
  try {
    ...
    if (!resp.ok) {
      return {
        ok: false as const,
        msg: errorMessage(body, 'Error al actualizar la opción'),
      };
    }
    ...
    return { ok: false as const, msg: 'Error al actualizar la opción' };
  }
}
